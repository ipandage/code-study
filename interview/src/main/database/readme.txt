...
但是如果在area、age两列上创建复合索引的话将带来更高的效率。如果我们创建了(area, age, salary)的复合索引，那么其实相当于创建了(area,age,salary)、(area,age)、(area)三个索引，
...
！=、 <> 、 like  、 in 、not in 、null 值判断、or 条件、字段表达式操作、字段函数操作
减小索引长度

如果是符合索引，必须使用到索引第一个字段作为条件时才能保证系统使用该索引，否则索引不被使用，尽量让字段顺序与索引顺序一致

exists 替换 in

索引并不是越多越好 提升查询效率 降低了insert update效率，表的索引数不要太多

尽量使用数字类型字段

不要使用select * from t 用具体的字段代替 *


mysql int(10) 10代表 显示最大宽度
